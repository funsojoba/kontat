...
const UserProfile = ({ userData, getUserAvatar, userAvatarData }) => {
    const [openModal, setOpenModal] = useState(false)
    const [openResetPassword, setResetPassword] = useState(false)

    const { first_name, last_name, email } = userData.user

    const firstInitial = first_name ? first_name.slice(0, 1) : null;
    const lastInitial = last_name ? last_name.slice(0, 1) : null;
    return <Body>
                ...
                <Formik
                    ...
                    {({ touched, errors, values, handleChange, handleSubmit, handleBlur }) => (
                        <Form onSubmit={handleSubmit}>
                            ...
                            <Button type="submit">{userAvatarData && userAvatarData.loading ? <Loader color="#fff" /> : 'upload'}</Button>
                        </Form>
                    )}
                </Formik>
            </Modal>
...
const mapStateToProps = (store) => ({
    ...
    userAvatarData: store.userAvatarReducer
})
